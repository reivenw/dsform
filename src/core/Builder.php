...
class Builder
{
  ...
  public static function validateData(array $property): void
  {
    $typeChecks = [
      ...
      'string' => ['label', 'description', 'title', 'text', 'html', 'type'],
      'callable' => ['output'],
      'mix' => ['name' => ['string', 'bool']]
    ];
        ...
        if (in_array($key, $keys)) {
          if ($type !== 'mix') {
            ...
            foreach ($value as $key2 => $value2) {
              if (is_string($key2) && !in_array(gettype($value2), $value[$key2])) {
                throw new \Exception('The value of ' . $key . ' must be a ' . implode(' or ', $value[$key2]));
              }
            }
          }
        }
